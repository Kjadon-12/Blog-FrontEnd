...
const UpdatePassword = () => {
  ...
  const {loading, isPasswordUpdated , appErr , serverErr} = user;

  const navigate = useNavigate();
  if(isPasswordUpdated){
    navigate(-1)  //"/profile/${userAuth?._id}"
  }
  ...
  return (
    <div className="min-h-screen bg-gray-700  flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        {appErr|| serverErr ? <h1 className="text-lg text-red-500 text-center pt-2">{serverErr} {appErr}</h1>:null}
      </div>
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <form className="space-y-6" onSubmit={formik.handleSubmit}>
            ...
            <div>
             ...
             (<button
             type="submit"
             className="inline-flex bg-indigo-700 justify-center w-full px-4 py-2 border border-indigo-300 shadow-sm text-sm font-medium rounded-md text-gray-200  hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500"
           >
